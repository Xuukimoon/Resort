...
static void FaintFromFieldPoison(u8 monIndex)//sobrevivir veneno 1PS
{
...
static bool32 MonFaintedFromPoison(u8 monIndex)
{
    ...
    // UB: Too few arguments for function 'GetMonData'
    if (IsMonValidSpecies(mon) && GetMonData(mon, MON_DATA_HP) == 1//0 //sobrevivir veneno 1PS
...
s32 DoPoisonFieldEffect(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
         ...
         && GetPrimaryStatus(GetMonData(mon, MON_DATA_STATUS)) == STATUS_PRIMARY_POISON)
        {
            ...
            if (hp != 1)//0//sobrevivir veneno 1PS
                hp--;
            if (hp == 1)//0//sobrevivir veneno 1PS
                numFainting++;
